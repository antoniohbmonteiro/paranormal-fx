...
3. Commit the source, metadata, and documentation changes. Do not commit `dist/`.
...
`dist/` is generated by the build executed inside `npm run check`. It is ignored by
Git and is included only in the installable release package. The packaging step
fails if `dist/main.js` was not generated or is missing from `paranormal-fx.zip`.
